...
export default async function TimelineDetail({ params }) {
  ...
  const [rows, files] = await Promise.all([
    sql`SELECT t.*, u.name AS author_name, u.position AS author_position, u.affiliation AS author_affiliation
        FROM timeline t LEFT JOIN users u ON u.id = t.author_id WHERE t.id = ${params.id}`,
  ...
  return (
    <>
      ...
      <article className="detail">
        ...
        <div className="detail-author">작성: {userLabel(item.author_name, item.author_position, item.author_affiliation)} · 등록 {fmtDateTime(item.created_at)}</div>
                ...
                <div className="attach" key={f.id}>
                  {isImg(f.mimetype) ? (
                    <a href={f.url} target="_blank" rel="noreferrer">
                      ...
                      <img src={f.url} alt="첨부 이미지" loading="lazy" />
                    </a>
                  ) : (
                    <a href={f.url} target="_blank" rel="noreferrer" className="attach-file">📄 파일 열기</a>
                  )}
                </div>
              ))}
            </div>
