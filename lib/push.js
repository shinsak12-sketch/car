...
// 알림을 켠 사람 목록 (사람별 기기 수). push_subscriptions 없으면 빈 배열.
export async function listSubscribers() {
  try {
    return await sql`SELECT COALESCE(NULLIF(user_name,''),'(이름없음)') AS name, COUNT(*)::int AS devices
                     FROM push_subscriptions GROUP BY user_name ORDER BY name`;
  } catch {
    return [];
  }
}
